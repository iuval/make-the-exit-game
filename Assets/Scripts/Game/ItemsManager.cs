...
public class ItemsManager : MonoBehaviour
{
	...
	public void LoadLevel(int level) {
		...
		float[] chances = Levels.ItemChances(level);
		foodWhen = chances[0];
		specialWhen = chances[1];
		rareWhen = chances[2];
		legendaryWhen = chances[3];
...
//				Debug.Log("New at: " + x + "," + newY);
	...
	private void OnDrag (Vector2 position)
	{
		...
		if (!isDragging && (elapsedTime > .4f || onPressPos != onDragPos)) {
			isDragging = true;
			tapCount = 0;
		...
		if (tapCount == 2) {
			...
			if (box.isClickable) {
				...
				if (box.Element == Elements.IronSword || box.Element == Elements.GoldSword || box.Element == Elements.DiamSword) {
					...
					dude.SetArmor (box);
				} else if (box.Element == Elements.Food || box.Element == Elements.Heart) {
					...
					GameManager.door.AddKey(box.gameObject);
				}		
